...
export class G3Module extends BaseModule<G3StatsData|void> {
  ...
  async retrieveData() {
    ...
    const g3Ref = this.getShaForBranchLatest('g3');
    /** The latest sha for the master branch. */
    const masterRef = this.getShaForBranchLatest('master');

    if (!g3Ref && !masterRef) {
      debug('Exiting early as either the g3 or master was unable to be retrieved');
      return this.resolve();
    }


    this.resolve(this.getDiffStats(g3Ref!, masterRef!, includeFiles, excludeFiles));
  }
  ...
  private getShaForBranchLatest(branch: string) {
    ...
    const fetchResult = this.git.runGraceful([
      'fetch', '-q', `https://github.com/${this.git.remoteConfig.owner}/${this.git.remoteConfig.name}.git`,
      branch
    ]);
   ...
   * files.
   */
  private getDiffStats(g3Ref: string, masterRef: string, includeFiles: string[], excludeFiles: string[]) {
  ...
  checkMatchAgainstIncludeAndExclude(file: string, includes: string[], excludes: string[]) {
    return (
