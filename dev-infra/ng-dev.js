...
function builder$c(yargs) {
    return tslib.__awaiter(this, void 0, void 0, function* () {
        return addGithubTokenOption(yargs);
    });
}
/** Handles the command. */
function handler$c({ githubToken }) {
    return tslib.__awaiter(this, void 0, void 0, function* () {
        ...
        if (git.hasUncommittedChanges()) {
            error(red(`Local working repository not clean. Please make sure there are no uncommitted changes.`));
            process.exitCode = 1;
            return;
        }
        /** Version numbers of the current (old) and new yarn installs. */
        const yarnVersion = {
            old: child_process.spawnSync('yarn', ['-v'], spawnSyncOpts).stdout.trim(),
            new: child_process.spawnSync('yarn', ['info', 'yarn@latest', 'dist-tags.latest'], spawnSyncOpts).stdout.trim()
        };
        /* File path of old yarn version */
        const oldYarnPath = path.join(getRepoBaseDir(), `.yarn/releases/yarn-${yarnVersion.old}.js`);
        ...
        try {
            ...
            child_process.spawnSync('yarn', ['policies', 'set-version', 'latest'], spawnSyncOpts);
            info(`Removing previous yarn version (${yarnVersion.old})`);
            fs.unlinkSync(oldYarnPath);
             ...
             */
            const env = Object.assign(Object.assign({}, process.env), { YARN_IGNORE_PATH: '0', 'PATH': process.env['PATH'].replace(/\/tmp\/yarn.+?\:/, '') });
            ...
            const createPrRequest = yield git.github.pulls.create({
                owner,
                repo,
                title,
                body,
            ...
            yield git.github.issues.addLabels({
                owner,
                repo,
 ...
 */
/** Build the parser for the caretaker commands. */
function buildMiscParser(yargs) {
