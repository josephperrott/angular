 ...
 *
 * @param testConfigPath Path (relative to the `test_cases` directory) of the `tests.json` file that
 *     describes the tests.
 */
export function generateGoldenPartial(testConfigPath: string): void {
  ...
  for (const test of tests) {
    const fs = initMockTestFileSystem(test.testPath);
...
function* compilePartials(fs: FileSystem, test: ComplianceTest): Generator<PartiallyCompiledFile> {
  ...
  for (const generatedPath of compileTest(
           fs, test.testPath, test.inputFiles, test.compilerOptions,
 ...
 */
function writePartial(files: PartiallyCompiledFile[]): void {
